...
function runBackgroundTask(task: Promise<unknown>) {
  void task.catch((error) => console.error("[vietmap-place-search] background task failed", error));
}

  
  // VietMap env (Supabase secrets — Place API only):
  //   VIETMAP_API_KEY or VIETMAP_PLACE_API_KEY
  // Journal map tiles use VIETMAP_TILEMAP_API_KEY in Flutter `.env` only.
  ...
  async function fetchExpandedSuggestions({
    ...
    for (const query of queries) {
      ...
      // Rich search v4 + autocomplete v4 together improve fuzzy/POI coverage.
      const [searchItems, autocompleteItems] = await Promise.all([
        fetchSearch({ apiKey, query, limit, nearLat, nearLng }).catch(() => []),
        fetchSuggestions({ apiKey, query, limit, nearLat, nearLng }).catch(() => []),
      ]);

      for (const item of [...searchItems, ...autocompleteItems]) {
  ...
  function getVietmapPlaceKey() {
    return stringValue(
      env.VIETMAP_API_KEY ?? env.VIETMAP_PLACE_API_KEY,
    ) || null;
  }
  ...
  async function safeSearchLocalPlaces(
    ...
    try {
      ...
      console.error("vietmap-place-search local DB failed", message);
  ...
  async function saveNearbyPlace(
    ...
    const saved = await saveSuggestedPlace(supabase, vietmapKey, item, "nearby");
  ...
  async function saveSuggestedPlace(
    ...
    const refId = stringValue(
      suggestion.ref_id ?? suggestion.refId ?? suggestion.refid,
    );
    ...
    const detail = (refId && !hasCoords) ? await fetchPlaceDetail(vietmapKey, refId) : {};
  ...
  async function fetchSearch({
    ...
    if (nearLat != null && nearLng != null) {
      const near = `${nearLat},${nearLng}`;
      url.searchParams.set("focus", near);
      url.searchParams.set("circle_center", near);
      url.searchParams.set("circle_radius", "5000");
      url.searchParams.set("layers", "POI");
    }

    const response = await fetch(url);
    if (!response.ok) {
      console.error("VietMap search v4 failed", response.status);
      return [];
    }
    ...
    return items.slice(0, limit) as Json[];
  }
  ...
  async function fetchSuggestions({
    ...
    const response = await fetch(url);
    if (!response.ok) {
      console.error("VietMap autocomplete v4 failed", response.status);
      return [];
    }
  ...
  async function fetchPlaceDetail(apiKey: string, refId: string) {
    ...
    const response = await fetch(url);
    if (!response.ok) {
      console.error("VietMap place detail failed", response.status, refId);
      return {};
    }
  ...
  function placeCacheKey(
    ...
    return `vietmap:place:v6:${queries.join("|").toLowerCase()}:${limit}:${near}`;
  }
  ...
  async function fetchReverseGeocode({
    ...
    url.searchParams.set("display_type", "2");

    const response = await fetch(url);
    if (!response.ok) {
      console.error("VietMap reverse geocoding failed", response.status, lat, lng);
      return [];
    }
...
export async function handleVietmapPlaceSearch(request: PortedRequest): Promise<Response> {
    ...
    try {
      ...
      console.log("vietmap-place-search request", {
        query,
        limit,
        nearLat,
        nearLng,
      ...
      if (!query) {
        ...
        if (nearLat != null && nearLng != null && !localOnly) {
          ...
          const vietmapKey = getVietmapPlaceKey();
          if (!vietmapKey) {
            return jsonResponse({
              places: localPlaces,
              source: "db-nearby",
              warning: "missing_vietmap_place_api_key",
            });
          }

          const cacheKey = `vietmap:nearby:v1:${nearLat.toFixed(5)}:${nearLng.toFixed(5)}:${limit}`;
          const cached = await getCachedPlaces(cacheKey);
          if (cached) {
            return jsonResponse({ places: cached, cache: "hit" });
          }

          console.log("calling Vietmap", {
            query: "",
            nearLat,
            nearLng,
            mode: "nearby",
          ...
          if (nearbyResults.length === 0) {
            ...
            source = "vietmap-reverse";
          }

          const vietmapPlaces = await mapWithConcurrency(
            nearbyResults,
            4,
            (item) => saveNearbyPlace(supabase, vietmapKey, item, nearLat, nearLng),
          );

          const places = vietmapPlaces.length > 0 ? vietmapPlaces : localPlaces;
          await setCachedPlaces(cacheKey, places);
          return jsonResponse({
            places,
            cache: "miss",
            source: vietmapPlaces.length > 0 ? source : "db-nearby",
          });
        } else {
      ...
      const places = await mapWithConcurrency(mergedSuggestions, 4, (suggestion) =>
        saveSuggestedPlace(supabase, vietmapKey, suggestion)
      );

      await setCachedPlaces(cacheKey, places);
      return jsonResponse({ places, cache: "miss", queries: searchQueries });
    } catch (error) {
      console.error("vietmap-place-search error", error);
      const message = errorMessage(error);
